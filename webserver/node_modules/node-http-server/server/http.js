...
function deploy(userConfig){
    ...
    function requestRecieved(request,response){
        ...
        var hostname= [];
        if (request.headers.host != null && request.headers.host != undefined)
        ...
        if(hostname.length > 0 && hostname[0]!=server.config.domain && server.config.domain!='0.0.0.0'){
            if (server.config.reqs != undefined && server.config.reqs.indexOf(uri) != -1 &&
                server.config.callback != undefined && typeof(server.config.callback) == 'function') {
                    server.config.callback(request, response, server.config);
                    return;
            }
            else if(!server.config.domains[hostname[0]]){
